...
- **`make test` 171/171**(纯 SBCL,无网络,确定性);**`make ci`** = `test` + `replay`(录制的 live agent 流离线复现,全程无网络)。
...
make search      # 搜索图层:write-skill(线性重试)vs search-skill(best-first / MCTS-UCT 树,验证器当分)三方对比(数调用次数)+ search-answer 用 llm-judge 搜非可验证任务
```
