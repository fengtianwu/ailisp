...
- **`make test` 156/156**(纯 SBCL,无网络,确定性);**`make ci`** = `test` + `replay`(录制的 live agent 流离线复现,全程无网络)。
- 实现:`src/`(reader / schema / safe-eval / repel / fallback / parallel / nl / replay / mcp / skill(+skill-agent) / model / ai / agent / rag / build / patterns / wolfram / sql / intent / skills),`bench/`(BFCL + 组合性基准),`tests/`(含 `fixtures/`),`examples/`(MCP 示例 server),`demo.lisp` / `showcase.lisp` / `repl.lisp`。
...
> **已落地**:**一个真实 agent——用 ailisp 写 Cadence SKILL**。SKILL 是 Cadence 自家的 Lisp 方言(EDA 自动化脚本),所以把上面所有零件用到一个真实目标语言上:`src/skill.lisp` 在 CL 里实现了 SKILL **list 记法子语言**的读取器+求值器+linter(算术/比较/逻辑、`if/cond/when/unless/for/foreach/while/let/setq/progn`、列表与字符串、`sprintf`),`write-skill`(`src/skill-agent.lisp`)让模型**写一段 SKILL 过程 → `skill-lint`(解析+未知算子,对绑定形结构感知)→ `skill-run` 在沙箱里跑样例 → 拿到确切故障反馈自愈**——就是 `intent`/`build`/`repel` 那条"合成→验证→修复"回路,只是验证器是个**真解释器**而非 CL eval。retry **逐次升温**(temp 0→0.35→…),破解小模型 temp-0 复读同一错误答案的陷阱(见 api-todo #8);`for` 语法错误给**带正例的精准诊断**,把模型从 `(for (i 0 w) ..)` 掰回 `(for i 0 (difference w 1) ..)`。live 三例全过:阶乘、Gauss 求和、`busNets`(按总线位宽生成 `D<0> D<1> D<2>` 网名——真 EDA 味道)。19 条确定性单测(求值器/linter/自愈回路,mock 演模型)。`make skill`。
